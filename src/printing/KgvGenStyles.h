 ...
 *
 * Each instance of KgvGenStyles is a collection of styles whose names
 ...
 *
 * KgvGenStyles features sharing, creation on demand, and name generation.
 * Since this is used for saving only, it doesn't feature refcounting, nor
 * removal of individual styles.
 *
 * NOTE: the use of KgvGenStyles isn't mandatory, of course. If the application
...
public:
     ...
     */
    QString lookup( const KgvGenStyle& style, const QString& name = QString::null, int flags = NoFlag );

    typedef QMap<KgvGenStyle, QString> StyleMap;
    /**
    ...
    struct NamedStyle {
        const KgvGenStyle* style; ///< @note owned by the collection
        QString name;
    };
    /**
     ...
     * Use this for saving the styles
     *
     * @param type the style type, see the KgvGenStyle constructor
     ...
     */
    const KgvGenStyle* style( const QString& name ) const;
     ...
     * be affected.
     */
    KgvGenStyle* styleForModification( const QString& name );
     ...
     *
     * Equivalent to using KgvGenStyle::setAutoStyleInStylesDotXml() but this can be done after lookup.
     *
     * This operation can't be undone; once styles are promoted they can't go back
     * to being content.xml-only.
     *
     * @see styles, KgvGenStyle::setAutoStyleInStylesDotXml
     */
 ...
 * Instances of KgvGenStyle can either be held in the KgvGenStyles collection,
 * or created (e.g. on the stack) and given to KgvGenStyles::lookup.
 *
 * @author David Faure <faure@kde.org>
 */
class KgvGenStyle
...
public:
    /**
     * Possible values for the "type" argument of the KgvGenStyle constructor.
    ...
    /**
     * Start the definition of a new style. Its name will be set later by KgvGenStyles::lookup(),
     ...
     */
    explicit KgvGenStyle( int type = 0, const char* familyName = 0,
     ...
     * they are saved in styles.xml, and styles.xml must be independent from content.xml.
     *
     * The application should use KgvGenStyles::styles( type, true ) in order to retrieve
    ...
    enum PropertyType
    {
        ...
        N_NumTypes ///< @internal - warning, adding items here affects binary compatibility (size of KgvGenStyle)
    };
     ...
     * KgvXmlWriter to generate it:
     * @code
     * QBuffer buffer;
     * buffer.open( IO_WriteOnly );
     * KgvXmlWriter elementWriter( &buffer );  // TODO pass indentation level
     ...
     *  Another solution would have been a qdict and a key() here, a la KgvTextFormat,
     ...
     */
    bool operator<( const KgvGenStyle &other ) const;
