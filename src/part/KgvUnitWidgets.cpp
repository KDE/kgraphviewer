...
QValidator::State KgvUnitDoubleValidator::validate(QString &s, int &pos) const
{
...
KgvUnitDoubleSpinBox::KgvUnitDoubleSpinBox(QWidget *parent)
    ...
    , KgvUnitDoubleBase(KgvUnit::U_PT, 2)
...
KgvUnitDoubleSpinBox::KgvUnitDoubleSpinBox(QWidget *parent, double lower, double upper, double step, double value, KgvUnit::Unit unit, unsigned int precision)
    ...
    , m_lowerInPoints(lower)
    , m_upperInPoints(upper)
    , m_stepInPoints(step)
{
    setMinimum(lower);
    setMaximum(upper);
...
KgvUnitDoubleLineEdit::KgvUnitDoubleLineEdit(QWidget *parent, double lower, double upper, double value, KgvUnit::Unit unit, unsigned int precision)
    : QLineEdit(parent)
    , KgvUnitDoubleBase(unit, precision)
    , m_value(value)
    , m_lower(lower)
...
void KgvUnitDoubleLineEdit::changeValue(double value)
{
    m_value = value < m_lower ? m_lower : (value > m_upper ? m_upper : value);
    setText(getVisibleText(m_value));
}

void KgvUnitDoubleLineEdit::setUnit(KgvUnit::Unit unit)
{
...
bool KgvUnitDoubleLineEdit::eventFilter(QObject *o, QEvent *ev)
{
#if 0
...
void KgvUnitDoubleComboBox::insertItem(double value, int index)
{
    QComboBox::insertItem(index, getVisibleText(value));
}

void KgvUnitDoubleComboBox::slotActivated(int index)
{
...
bool KgvUnitDoubleComboBox::eventFilter(QObject *o, QEvent *ev)
{
#if 0
	if( ev->type() == QEvent::FocusOut || ev->type() == QEvent::Leave || ev->type() == QEvent::Hide )
	{
    ...
    , m_step(1.0)
{
    ...
    connect(down, &QPushButton::clicked, this, &KgvUnitDoubleSpinComboBox::slotDownClicked);
    ...
    connect(m_combo, &KgvUnitDoubleComboBox::valueChanged, this, &KgvUnitDoubleSpinComboBox::valueChanged);
    layout->addWidget(m_combo, 0, 2, 2, 1);
}

KgvUnitDoubleSpinComboBox::KgvUnitDoubleSpinComboBox(QWidget *parent, double lower, double upper, double step, double value, KgvUnit::Unit unit, unsigned int precision)
    : QWidget(parent)
    , m_step(step) //, m_lowerInPoints( lower ), m_upperInPoints( upper )
{
    QGridLayout *layout = new QGridLayout(this);
    // layout->setMargin( 2 );
    QPushButton *up = new QPushButton("+", this);
...
void KgvUnitDoubleSpinComboBox::slotUpClicked()
{
...
void KgvUnitDoubleSpinComboBox::insertItem(double value, int index)
{
    m_combo->insertItem(value, index);
}

void KgvUnitDoubleSpinComboBox::updateValue(double value)
{
    m_combo->updateValue(value);
}

double KgvUnitDoubleSpinComboBox::value() const
{
