...
void attrid(char const* first, char const* last)
{
  if (phelper) 
  {
    ...
    phelper->attrid = id;
    phelper->valid = "";
//     qDebug() << "Got attr ID  = '"<<QString::fromStdString(phelper->attrid)<<"'";
  }
}
...
//     qDebug() << "Got subgraph id = '"<<QString::fromStdString(phelper->subgraphid)<<"'";
  }
}
...
void valid(char const* first, char const* last)
{
  if (phelper) 
  {
    ...
    if (id.size()>0 && id[id.size()-1] == '"') id = id.substr(0,id.size()-1);
    phelper->valid = id;
//     qDebug() << "Got attr val = '"<<QString::fromStdString(phelper->valid)<<"'";
  }
}
...
{
  qDebug() << "setgraphattributes with z = " << phelper->z;
  if (phelper) 
  {
...
{
  qDebug() << "setnodeattributes with z = " << phelper->z;
  if (phelper) 
  {
...
{
  qDebug() << "edgebound: " << QString::fromStdString(std::string(first,last));
  if (phelper) 
  {
    std::string id(first,last);
    if (id.size()>0 && id[0] == '"') id = id.substr(1);
...
{
  qDebug() << "Parsing spline..." << QString::fromStdString(str);
  char e = 'n', s = 'n';
  int sx,sy,ex,ey;
  QPair< float, float > p;
  bool res;
...
void valid_op(char const* first, char const* last)
{
  ...
  qDebug() << "Validating render operation '"<<QString::fromStdString(s)<<"': '"<<renderop.renderop<<"/"<<renderop.str<<"'";
